...
public class App_12_stream_downstream {
    public static void main(String[] args) {
        ...
        final List<Album> albums = asList(
                Album.of(cobain, "Nevermind"),
                Album.of(reynolds, "Evolve"),
                Album.of(cobain, "In Utero"));

        final Map<Musician, List<String>> musicianToAlbums = albums.stream()
                .collect(Collectors.groupingBy(Album::getMainMusician,
                        Collectors.mapping(Album::getTitle, Collectors.toList())));
